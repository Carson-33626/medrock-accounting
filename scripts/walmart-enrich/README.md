...
npx tsx scripts/walmart-enrich/run-cdp.ts

# 2. SPLIT + ATTACH — cache -> Ramp. Dry-run by default.
npx tsx scripts/walmart-enrich/run-cdp-split.ts
npx tsx scripts/walmart-enrich/run-cdp-split.ts --entity TN --cap 1 --live    # pilot one entity
npx tsx scripts/walmart-enrich/run-cdp-split.ts --live                        # all entities, uncapped
```
