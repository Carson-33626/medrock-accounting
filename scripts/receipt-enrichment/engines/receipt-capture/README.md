...
```
npx tsx scripts/receipt-enrichment/engines/receipt-capture/uline-bootstrap.ts --entity=FL
```

The script polls until you reach an authenticated Order History page (up to 10 minutes), reads
the `#CompanyName` header, and saves `storageState` to `scripts/receipt-enrichment/cache/receipt-capture/.state/uline-<ENT>.json`
...
```
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-sweep.ts [--dry-run] [--vendor toprx,uline,amazon,walmart,amazon-csv] [--limit N] [--skip-scan]
```
...
**Outputs** (under `scripts/receipt-enrichment/cache/receipt-capture/out/sweep/`):
...
- **ULINE bootstrap**: if `run-uline.ts` exits 2 for any entity, that session is stale or
  missing — re-run `npx tsx scripts/receipt-enrichment/engines/receipt-capture/uline-bootstrap.ts --entity=<ENT>` to sign
...
- **Walmart CDP Chrome**: to extract Walmart order history, a headless Chrome instance must be
  ...
  sweep (or run `npx tsx scripts/receipt-enrichment/engines/walmart-enrich/run-cdp.ts` directly).
- **Amazon-CSV extract per Business account**: extraction happens outside the sweep because it
  ...
  `npx tsx scripts/receipt-enrichment/engines/amazon-csv-enrich/run-extract-txns.ts --account FL` (then `TN`, `TX` — the
  ...
  attached: `npx tsx scripts/receipt-enrichment/engines/amazon-csv-enrich/fetch-invoices.ts`. The sweep's
...
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-toprx.ts [--entity=FL] [--since 2025-09-01] [--live] [--limit 5]

# ULINE — --entity is REQUIRED (no all-entity default); accepts a comma list for joint mode
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-uline.ts --entity=FL [--since 2025-09-01] [--live] [--limit 5] [--csv path/to/MyOrderHistory.csv] [--window 3]
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-uline.ts --entity=FL,TN [--since ...] [--live] [--limit 5]   # joint mode — see above
```
...
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-amazon.ts [--entity=FL] [--pages 100]

# Live pilot — capped, audited. Get Carson's go-ahead first (see "Human-gated procedures" above).
npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-amazon.ts --entity=FL --live --limit 5
```
...
2. Pilot small: `npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-toprx.ts --live --limit 5` (or the ULINE
...
1. `npx tsx scripts/receipt-enrichment/engines/receipt-capture/uline-bootstrap.ts --entity=TN` (then `TX`) — log in by hand
...
4. Dry-run to confirm: `npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-uline.ts --entity=TN` should log
