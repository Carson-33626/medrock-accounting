// scripts/receipt-enrichment/engines/receipt-capture/toprx-session.ts
...
// Sessions live in the consolidated cache (../../cache/receipt-capture/.state), NOT next to this
// module. This was `join(__dirname, '.state')` before the 2026-08-07 consolidation — a form that
// survives a folder move without erroring, silently creating a fresh empty session store beside
// the module while the panel reads the real one. Deriving from paths.ts makes that impossible.
...
export async function withTopRxPage<T>(entity: Entity, fn: (page: Page) => Promise<T>): Promise<T> {
  ...
  const statePath = sessionPath('toprx', entity);
