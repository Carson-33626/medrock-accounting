// scripts/receipt-enrichment/engines/receipt-capture/uline-session.ts
//
...
// scripts/receipt-enrichment/engines/walmart-enrich/session.ts). Bootstrap once per entity (headed, human does the ENTIRE
...
// See toprx-session.ts: sessions live in the consolidated cache, not next to this module.
...
export function ulineStatePath(entity: Entity): string {
  return sessionPath('uline', entity);
}
