...
// itemization. Use scripts/receipt-enrichment/engines/receipt-capture/run-amazon.ts to un-split + attach order-ID memo instead.
...
//
//   cd web && npx tsx scripts/receipt-enrichment/engines/amazon-enrich/run.ts                 # dry-run, all entities (RETIRED: --live refused)
//   cd web && npx tsx scripts/receipt-enrichment/engines/amazon-enrich/run.ts --basket        # Track A: basket vendors (Amazon excluded)
//   cd web && npx tsx scripts/receipt-enrichment/engines/receipt-capture/run-amazon.ts --live # ← Replace with this for Amazon un-split + order-ID
...
async function main(): Promise<void> {
  ...
  // scripts/receipt-enrichment/engines/receipt-capture/run-amazon.ts and the 2026-07-28 amazon-unsplit spec). Live Amazon
