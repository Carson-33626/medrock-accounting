...
//   npx tsx scripts/receipt-enrichment/engines/amazon-csv-enrich/run-extract.ts --account <label> [--business "<name>"] [--no-switch] [--span PAST_12_MONTHS] [--limit N] [--skip-invoices]
...
async function main(): Promise<void> {
  ...
  const OUT = `${ACSV.out}/${account}`;
  const PDF_DIR = `${ACSV.receipts}/${account}`;
