// web/scripts/receipt-enrichment/engines/receipt-capture/sweep-ui-status.ts
//
...
import { checkTopRx, checkLetco, checkMedisca, checkCdp } from '../engines/receipt-capture/sweep-preflight';
import { txnReportPath } from '../engines/amazon-csv-enrich/paths';
import { ALL_ENTITIES } from '../engines/ramp-split-push/types';
import { SAMS } from '../engines/walmart-enrich/retailer-profile';
...
/**
 * Which section of the panel a card belongs to. The two groups behave differently enough that a
 * flat nine-card list buried the distinction: 'receipts' pipelines attach receipts to Ramp CARD
 * transactions, 'bills' pipelines code or create vendor BILLS in QuickBooks.
 */
export type VendorGroup = 'receipts' | 'bills';
...
export interface PanelStatus {
  ...
  /**
   * Display metadata for every action name referenced by the cards above. Shipped with the status
   * so the page can render a live action as a distinct, arm-gated button instead of inferring
   * intent from its kebab-case name (which is what made every live button a guaranteed 400).
   */
  actionMeta: Readonly<Record<string, ActionMeta>>;
}
...
const ULINE_STATE_DIR = RC.state;
const SWEEP_OUT = RC.sweep;
const WALMART_CACHE = `${WM.out}/extraction-cache.json`;
const AMAZON_CSV_ACCOUNTS = ['FL', 'TN', 'TX'] as const;
const AMAZON_CSV_PDF_DIR = ACSV.sharedPdf;
...
function toprxCard(deps: StatusDeps): VendorCard {
  ...
  return { key: 'toprx', label: 'TopRx', group: 'receipts', light, detail: r.detail, actions: [] };
}
...
function ulineSessionCard(key: string, label: string, file: string, actions: string[], deps: StatusDeps): VendorCard {
  ...
  if (!info || !info.exists) return { key, label, group: 'receipts', light: 'red', detail: 'no session — needs bootstrap', actions };
  const days = ageDays(info.mtimeMs, deps.now());
  if (days >= STALE_DAYS) return { key, label, group: 'receipts', light: 'amber', detail: `session ${days}d old — re-bootstrap recommended`, actions };
  return { key, label, group: 'receipts', light: 'green', detail: `session ${days}d old`, actions };
}
...
function amazonCard(): VendorCard {
  ...
  return { key: 'amazon', label: 'Amazon (API)', group: 'receipts', light: 'green', detail: 'API-only, no session needed', actions: [] };
}
...
function mediscaCard(deps: StatusDeps): VendorCard {
  ...
  const caches = ALL_ENTITIES
    .map((e) => deps.statePath(`${RC.out}/medisca-cache-${e}.json`))
