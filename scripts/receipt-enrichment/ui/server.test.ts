// web/scripts/receipt-enrichment/engines/receipt-capture/sweep-ui-server.test.ts
//
...
import { runChild as realRunChild } from '../engines/receipt-capture/sweep-exec';
import type { ChildResult } from '../engines/receipt-capture/sweep-exec';
import type { ResolvedAction, ActionError } from './actions';
...
describe('startSweepUiServer', () => {
  ...
  it('GET /api/status returns 200 with the documented PanelStatus JSON shape', async () => {
    ...
    try {
      ...
      expect(json.vendors).toHaveLength(9);
