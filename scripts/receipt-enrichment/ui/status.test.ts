...
describe('assembleStatus — never touches real fs/network (fully-faked deps)', () => {
  ...
  describe('uline-FLTN light (FL session age; TN rides FL)', () => {
    ...
    it('green when FL session is fresh (< 14 days old)', async () => {
      const statePath = fileMap({ [`${RC.state}/uline-FL.json`]: { exists: true, mtimeMs: daysAgo(2) } });
    ...
    it('amber when FL session is exactly 14 days old (boundary)', async () => {
      const statePath = fileMap({ [`${RC.state}/uline-FL.json`]: { exists: true, mtimeMs: daysAgo(14) } });
      const status = await assembleStatus(baseDeps({ statePath }));
      expect(cardByKey(status, 'uline-FLTN').light).toBe('amber');
    });
    it('amber when FL session is older than 14 days', async () => {
      const statePath = fileMap({ [`${RC.state}/uline-FL.json`]: { exists: true, mtimeMs: daysAgo(30) } });
      const status = await assembleStatus(baseDeps({ statePath }));
      expect(cardByKey(status, 'uline-FLTN').light).toBe('amber');
    });
  });
  ...
  describe('uline-TX light (independent of FL/TN)', () => {
    it('red when no TX session exists even if FL is fresh', async () => {
      const statePath = fileMap({ [`${RC.state}/uline-FL.json`]: { exists: true, mtimeMs: daysAgo(1) } });
      const status = await assembleStatus(baseDeps({ statePath }));
    ...
    it('green when TX session is fresh', async () => {
      const statePath = fileMap({ [`${RC.state}/uline-TX.json`]: { exists: true, mtimeMs: daysAgo(0) } });
      const status = await assembleStatus(baseDeps({ statePath }));
  ...
  describe('walmart light (CDP + cache)', () => {
    ...
    it('detail reports cache age when the extraction cache exists', async () => {
      const statePath = fileMap({ [`${WM.out}/extraction-cache.json`]: { exists: true, mtimeMs: daysAgo(3) } });
  ...
  describe("sam's club light (shares Walmart's Chrome, own cache)", () => {
    ...
    it('reads its OWN cache, not the Walmart one', async () => {
      const statePath = fileMap({
        [`${WM.out}/extraction-cache.json`]: { exists: true, mtimeMs: daysAgo(1) },
        [`${WM.out}/sams/extraction-cache.json`]: { exists: true, mtimeMs: daysAgo(9) },
      });
      const status = await assembleStatus(baseDeps({ statePath, cdpCheck: async () => ({ reachable: true, detail: 'up' }) }));
    ...
    it('says so when Sam\'s has no cache even though Walmart does', async () => {
      const statePath = fileMap({ [`${WM.out}/extraction-cache.json`]: { exists: true, mtimeMs: daysAgo(1) } });
      const status = await assembleStatus(baseDeps({ statePath, cdpCheck: async () => ({ reachable: true, detail: 'up' }) }));
  ...
  describe('amazon-csv light (per-account cache ages + invoice-pdf count)', () => {
    ...
    it('detail reports invoice-pdf count from the shared receipts cache', async () => {
      const status = await assembleStatus(baseDeps({ listDir: (dir) => (dir === ACSV.sharedPdf ? ['amazon-111.pdf', 'amazon-222.pdf'] : []) }));
...
// ── Grouping + action referential integrity (added 2026-08-07) ────────────────
describe('card grouping and action metadata', () => {
  it('assigns every card to a known group', async () => {
    const status = await assembleStatus(baseDeps());
    for (const card of status.vendors) {
      expect(`${card.key}:${card.group}`).toMatch(/:(receipts|bills)$/);
    }
  });

  it('puts the bill pipelines in bills and everything else in receipts', async () => {
    const status = await assembleStatus(baseDeps());
    const byGroup = (g: string): string[] => status.vendors.filter((v) => v.group === g).map((v) => v.key).sort();
    expect(byGroup('bills')).toEqual(['letco', 'medisca']);
    expect(byGroup('receipts')).toEqual(
      ['amazon', 'amazon-csv', 'sams', 'toprx', 'uline-FLTN', 'uline-TX', 'walmart'].sort(),
    );
  });

  it('references only actions that actually exist in the registry', async () => {
    // A card naming an action the registry does not have renders a button that always 400s —
    // exactly the class of bug this whole pass was fixing. Cheap to assert, so assert it.
    const status = await assembleStatus(baseDeps());
    for (const card of status.vendors) {
      for (const action of card.actions) {
        expect(`${card.key} -> ${action}`).toBe(
          `${card.key} -> ${ACTION_NAMES.includes(action) ? action : 'MISSING_FROM_REGISTRY'}`,
        );
      }
    }
  });

  it('ships metadata covering every action any card references', async () => {
    const status = await assembleStatus(baseDeps());
    for (const card of status.vendors) {
      for (const action of card.actions) {
        expect(status.actionMeta[action]).toBeDefined();
      }
    }
  });
});
