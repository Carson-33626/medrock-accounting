...
function safe(label: string, resolved: ResolvedAction): ActionDef {
  return { meta: { label, risk: 'safe', requiresArm: false }, build: () => resolved };
}
 ...
 */
function live(name: string, label: string, resolved: ResolvedAction): ActionDef {
  return {
    meta: { label, risk: 'live', requiresArm: true },
...
const REGISTRY: Record<string, ActionDef> = {
  ...
  'scan-only': safe('Scan (refresh counts)', { kind: 'scan' }),
};
