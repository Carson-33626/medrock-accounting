// web/scripts/receipt-enrichment/engines/receipt-capture/sweep-ui-server.ts
//
...
import { assembleStatus, defaultStatusDeps } from './status';
import { resolveAction as defaultResolveAction } from './actions';
import { makeAssetReader } from './assets';
import type { AssetReader } from './assets';
import type { ResolvedAction, ActionError, ActionRequestBody } from './actions';
import { runChild as defaultRunChild } from '../engines/receipt-capture/sweep-exec';
import { scanEntity } from '../engines/receipt-capture/sweep-scan';
import { rampToken } from '../engines/ramp-split-push/ramp-client';
...
export interface SweepUiServerOpts {
  port: number;
  host?: string;
  /** The panel HTML, served at GET /. Production reads ui/index.html; tests pass a stub. */
  page: string;
  /**
   * Serves the page's stylesheet and client script. Defaults to the real allowlisted reader over
   * this module's own directory; tests can substitute a fake, and a test that omits it simply gets
   * 404s for assets it never requests.
   */
  assets?: AssetReader;
...
export function startSweepUiServer(opts: SweepUiServerOpts): Promise<SweepUiServerHandle> {
  ...
  const assetsFn = opts.assets ?? makeAssetReader();
  ...
  async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    // Allowlisted static assets only (assets.ts) — `url` is never joined onto a directory, so
    // there is no traversal surface here despite this being the one route that reads files by
    // request path.
    if (req.method === 'GET') {
      const asset = assetsFn(url);
      if (asset) {
