...
describe('loadTxnReportCharges', () => {
  it('pools accounts and records which account each order id is visible under', () => {
    const r = loadTxnReportCharges(['FL', 'TN'], deps({
      'scripts/amazon-csv-enrich/out/FL/transactions.csv': report(row({ ref: 'F1', orderId: 'O-FL' })),
      'scripts/amazon-csv-enrich/out/TN/transactions.csv': report(row({ ref: 'T1', orderId: 'O-TN' })),
    }));
  ...
  it('de-dupes a payment ref appearing in two exports, first account wins', () => {
    const r = loadTxnReportCharges(['FL', 'TN'], deps({
      'scripts/amazon-csv-enrich/out/FL/transactions.csv': report(row({ ref: 'DUP', amount: '"1.00"' })),
      'scripts/amazon-csv-enrich/out/TN/transactions.csv': report(row({ ref: 'DUP', amount: '"2.00"' })),
    }));
  ...
  it('reports a missing report as missing instead of throwing, and still pools the rest', () => {
    const r = loadTxnReportCharges(['FL', 'TX'], deps({
      'scripts/amazon-csv-enrich/out/FL/transactions.csv': report(row({ ref: 'F1' })),
    }));
    expect(r.charges).toHaveLength(1);
    expect(r.missing).toEqual([{ account: 'TX', path: 'scripts/amazon-csv-enrich/out/TX/transactions.csv' }]);
  });
});
