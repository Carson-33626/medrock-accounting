...
// Run from web/ dir:  npx tsx scripts/receipt-capture/uline-bootstrap.ts --entity=FL
...
function entityArg(): Entity {
  ...
  throw new Error('Usage: npx tsx scripts/receipt-capture/uline-bootstrap.ts --entity=FL|TN|TX');
}

async function main(): Promise<void> {
  const entity = entityArg();
  mkdirSync(STATE_DIR, { recursive: true });
  const statePath = join(STATE_DIR, `uline-${entity}.json`);
