// scripts/receipt-capture/_verify-uline.ts
//
// Live verify for the ULINE storageState session provider (task 7 pivot). REQUIRES a bootstrapped
// session at scripts/receipt-capture/.state/uline-FL.json (run uline-bootstrap.ts --entity=FL
// once, by hand, if missing). Run from web/:
//   npx tsx scripts/receipt-capture/_verify-uline.ts
