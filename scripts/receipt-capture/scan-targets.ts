...
//
// Usage (from web/):
//   npx tsx scripts/receipt-capture/scan-targets.ts
//   npx tsx scripts/receipt-capture/scan-targets.ts --top 40
//   npx tsx scripts/receipt-capture/scan-targets.ts --all
//   npx tsx scripts/receipt-capture/scan-targets.ts --merchant Indeed     # drill into one merchant
//   npx tsx scripts/receipt-capture/scan-targets.ts --csv out/my-scan.csv
//
...
function parseArgs(argv: string[]): Args {
  ...
  return {
    ...
    csv: get('--csv') ?? 'scripts/receipt-capture/out/target-scan.csv',
  };
}
