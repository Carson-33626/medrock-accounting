...
import type { FlDr15Response, TxReturnResponse, TnReturnResponse } from '@/types/sales-tax';
import type { FlSourceRow } from './sales-tax-fl';
import type { TxSourceRow } from './sales-tax-tx';
import type { TnSourceRow } from './sales-tax-tn';
...
/**
 * PDF builder for the Tennessee SLS-450 annual return.
 * Page 1: the SLS-450 values + derivation, then the taxable-transaction detail.
 */
export async function buildTnReturnPdf(result: TnReturnResponse, sourceRows: TnSourceRow[]): Promise<Uint8Array> {
  const doc = await PDFDocument.create();
  const font = await doc.embedFont(StandardFonts.Helvetica);
  const bold = await doc.embedFont(StandardFonts.HelveticaBold);
  ...
  text('MedRock Tennessee', margin, y, { size: 11, font: bold, color: GRAY });
  y -= 22;
  text(`Sales & Use Tax Return (SLS-450) — CY${result.period}`, margin, y, { size: 18, font: bold, color: PURPLE });
  y -= 16;
  text(
    `Generated ${new Date().toISOString().slice(0, 10)} from the LifeFile sales-tax feed` +
      (result.feedAsOf ? ` (data as of ${result.feedAsOf.slice(0, 10)})` : '') +
      (result.diagnostics.partialYear ? ` · PARTIAL YEAR (months: ${result.diagnostics.monthsCovered.join(', ')})` : ''),
    margin,
    y,
    { size: 8, color: GRAY },
  );
  ...
  const lineRows: [string, number, boolean][] = [
    ['Gross Sales (Line 1)', b.grossSales, false],
    ['Exempt / Deductions', b.exemptSales, false],
    ['Taxable Sales', b.taxableSales, false],
    ['Taxable Purchases (use tax)', b.taxablePurchases, false],
    [`State Tax (${(b.stateTaxRate * 100).toFixed(2)}%)`, b.stateTaxDue, false],
    [`Local Tax (${(b.localTaxRate * 100).toFixed(2)}%)`, b.localTaxDue, false],
    ['Total Tax Due', b.totalTaxDue, true],
  ];
  text('SLS-450 Values', margin, y, { size: 12, font: bold });
  y -= 6;
  page.drawLine({ start: { x: margin, y }, end: { x: 612 - margin, y }, thickness: 1, color: PURPLE });
  y -= 18;
  for (const [label, value, hl] of lineRows) {
    text(label, margin, y, { size: 10, color: hl ? PURPLE : BLACK, font: hl ? bold : font });
    right(usd(value), 612 - margin, y, { size: 10, font: hl ? bold : font, color: hl ? PURPLE : BLACK });
    y -= 18;
  }
  y -= 14;
  ...
  page.drawLine({ start: { x: margin, y }, end: { x: 612 - margin, y }, thickness: 0.5, color: GRAY });
  y -= 16;
  const cols = { tx: margin, date: margin + 80, city: margin + 150, sub: 612 - margin - 90, tax: 612 - margin };
  text('Tx ID', cols.tx, y, { size: 8, font: bold, color: GRAY });
  text('Date', cols.date, y, { size: 8, font: bold, color: GRAY });
  text('City', cols.city, y, { size: 8, font: bold, color: GRAY });
  right('Taxable', cols.sub, y, { size: 8, font: bold, color: GRAY });
  right('Tax', cols.tax, y, { size: 8, font: bold, color: GRAY });
  y -= 13;
  for (const r of taxable) {
    ...
    text(r.tx_id, cols.tx, y, { size: 8 });
    text(r.date, cols.date, y, { size: 8 });
    text(r.city.slice(0, 18), cols.city, y, { size: 8 });
    right(usd(r.taxable_base), cols.sub, y, { size: 8 });
    right(usd(r.tax), cols.tax, y, { size: 8 });
    y -= 13;
  }
  ...
  text(
    `Full source data (all ${d.totalTransactions.toLocaleString()} TN transactions) is available in the CSV and XLSX exports.`,
    margin,
    y,
    { size: 8, color: GRAY },
  );
